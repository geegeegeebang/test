    ...
    <scroll class="list-view"  ref="listView" :listenScroll=true :probeType=3 @scroll="scroll">
        <ul>
          <li class="list-group" ref="lists" v-for="item in singers" :key="item.key">
              <h2 class="list-group-title">{{item.tittle}}</h2>
              <ul>
        ...
        <div class="list-shortcut" >
            <ul>
                <li class="item" v-for="(item,index) in cutList" :class="{'current': currentIndex == index}" :key="item.key" @touchstart="onTouchStart" @touchmove.stop.prevent="onTouchMove" :data-index="index" >{{item}}</li>
            </ul>
        </div>
...
export default {
  ...
  computed:{
      cutList(){
        ...
        for(let a in this.singers){
          arr.push(this.singers[a].tittle.substring(0,1))
        }
        return  arr   
      }
  },
  methods:{
    ...
    onTouchMove(ev){
      ...
      //计算手指滑过了几个li，并且向下取整
      let a1 = (this.touch.y1-this.touch.y) /18 |0
      let a2 = a1 + parseInt(this.touch.index)
      this.scrollTo(a2)
    },
    scroll(obj){
      this.scrollY = obj.y
